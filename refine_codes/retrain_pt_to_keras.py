...
if __name__ == "__main__":
	...
	k_model = onnx_to_keras(onnx_model, ['input'], name_policy='renumerate')#renumerate
	k_model.summary()
	input_np2 = np.random.uniform(0, 1, (1, 1, 30225))
	# input_np2 = tf.expand_dims(input_np2, 0)#NHWC
	input_np2 = np.transpose(tf.expand_dims(input_np2, 0).numpy(), [0, 3, 1, 2])#NCHW
	# k_in = np.reshape(input_np, (-1, 1,1,30225))
	pred = k_model.predict(input_np2)
	print(pred)
